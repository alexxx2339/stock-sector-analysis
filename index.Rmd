...
-   [➡️ View Python Dashboard](https://alexxx2339.github.io/stock-sector-analysis/stock_dashboard.html)
...
last_updated <- max(index(prices), na.rm = TRUE)

# Display it as a message
cat("📅 **Data last updated on:**", format(last_updated, "%B %d, %Y"))
```

```{rprices <- do.call(merge, lapply(tickers, function(x) Ad(get(x))))}
```
